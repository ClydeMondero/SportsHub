<style> <?php include "../styles/footer.css" ?></style>

<!--Footer-->
<footer>
    <div class="footer-container">
        <div class="information">
            <h1>ABOUT US</h1>
            <p>Welcome to Vulcan Sports Hub, where passion meets performance. 
                At Vulcan, we've crafted more than just a sports store; we've built a hub for enthusiasts, 
                athletes, and anyone embracing an active lifestyle.
            </p>
        </div>
        <div class="information">
            <h1>LOCATION</h1>
            <p>213 New York St. Cubao, Metro, Manila Philippines</p>
        </div>
        <div class="information">
            <h1>TALK TO PROFESSIONALS</h1>
            <div class="professionals-gmail">
               <a href="mailto:monderoclyde24@gmail.com" target="_blank"><div class="gmails"><i class="fa-solid fa-envelope"></i> Clyde Reyniel C. Mondero</div></a>
               <a href="mailto:andreipoma123@gmail.com" target="_blank"><div class="gmails"><i class="fa-solid fa-envelope"></i> Andrei John D. Poma</div></a>
                <a href="mailto:bhenzmharlbartolome" target="_blank"><div class="gmails"><i class="fa-solid fa-envelope"></i> Bhenz Mharl L. Bartolome</div></a>
            </div>
        </div>
    </div>
    <div class="footer-navigation">
        <div class="left-content">
            <div><i class="fa-solid fa-location-dot"></i> Philippines</div>
            <div class="gray-text"><i class="fa-solid fa-copyright"></i> 2023, Vulcan Sports Hub Inc. All Rights Reserved.</div>
        </div>
        <div class="right-content">
            <p class="gray-text"><a href="guides.php">Guide</a></p>
            <p class="gray-text"><a href="term-of-use.php">Term of Use</a></p>
            <p class="gray-text"><a href="terms-of-sale.php">Term of Sale</a></p>
            <p class="gray-text"><a href="privacy-policy.php">Vulcan Sports Hub Policy</a></p>
        </div>
    </div>
</footer>
